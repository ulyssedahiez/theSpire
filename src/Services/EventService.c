...
p_listeEvent genererListesEvents() {
    ...
    ajouterEventListe(listeEvents, creerEvent("Un piège magique modiﬁe le deck de Peter", "Transforme tous les strike en esquive", "Transforme toutes les esquives en strikes", 1));
    ajouterEventListe(listeEvents, creerEvent("La salle est vide, mais dans un (Charles) chaudron encore allumé, une potion est presque prête. Comment la terminer ?", "Faire une potion de santé (hp max +10)", "Faire une potion de mana (mana max +20)", 1));
